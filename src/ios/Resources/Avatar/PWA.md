...
- The native OpenMinis host may connect backend events to `window.SoulNestAvatar` under #12 without exposing native secrets to Web content.

Current `send`/microphone controls remain local demo behavior until the transport bridge is implemented. This is intentional: #18 provides the reusable/installable web package without duplicating HealthKit, HomeKit, Location, Calendar, Photos, Speech, Native Offloads or other OpenMinis capabilities in JavaScript.
