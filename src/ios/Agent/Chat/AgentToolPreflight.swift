...
/// Tool preflight validation + JSON-args repair, kept free of `AIChatViewModel`
/// so the unit-test target can compile it standalone.
enum AgentToolPreflight {
    ...
    static func validateToolCall(name: String,
                                 args: [String: Any],
        ...
        let enforced = toolDef.required.filter { !preflightNonBlockingFields.contains($0) }
        ...
        for field in enforced {
            ...
            if let s = raw as? String, s.isEmpty,
               !preflightEmptyStringAllowed(tool: name, field: field) {
                missing.append(field)
            }
        }
        if !missing.isEmpty {
